...
class TimerViewModel extends ChangeNotifier {
  ...
  /// 그럴때 static 을 사용하기도 하지만, 싱글톤이라는 패텬(기법)을 사용하여
  /// 이미 생성된 객체를 재활용(다시이용) 하는 방법이 있다.
  ...
  Future<int> getTimer() async {
    /// 이 클래스에서 만들어진 _prefs 객체를 private 로 사용하기 위하여
    /// 요구하기
    final SharedPreferences prefs = await _prefs;

    /// localStorage(SP)에 TIMER 이라는 이름으로 변수가 저장되어 있으면
  ...
  void setTimer(timer) async {
    ...
    final SharedPreferences prefs = await _prefs;
    // SP 에 TIMER 라는 이름으로 설정된 타이머 시작값을 저장하기
